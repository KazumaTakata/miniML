...
let rec skipWhitespace (lex : lexer) : lexer =
  if lex.if_end then lex
    ...
    match lex.ch with
    | ' ' | '\t' | '\n' | '\r' -> skipWhitespace (advanceLex lex)
    | _ -> lex

let rec readUntil (lex : lexer) (fn : char -> bool) : lexer =
  if lex.if_end then lex
  else if fn lex.ch then readUntil (advanceLex lex) fn
  else lex
...
let nextToken (lex : lexer) : token option * lexer =
  ...
  if newlex.if_end then (None, newlex)
  else
    ...
    | '[' | ']' ->
        ...
        in
        (Some token, advanceLex newlex)
    | _ ->
        if isLetter newlex.ch then
          let token_string, lexer = getNumberOrString newlex isLetterOrDigit in
          ...
          let token_string, lexer = getNumberOrString newlex isDigit in
